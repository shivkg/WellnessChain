...
//Email checking --kiran
$emailSame = "SELECT email FROM patient_data where pid= '".$pid."'";
    $emailSameResult = sqlStatement($emailSame);
    while ($frow = sqlFetchArray($emailSameResult)) {
        $emailId = $frow['email'];
    }
    if($emailId != $_POST['form_email']){
        $emailCheck = "SELECT email FROM patient_data where email = '".$_POST['form_email']."'";
           $emailcheckResult = sqlStatement($emailCheck);
            ...
            if(!empty($emailCheck)){
                ...
                <?php
                if($emailMsg){
                    echo "alert('" . addslashes($emailMsg) . "');\n";
                } 
                ?>
                </script>
                <?php 
