...
function echoServiceLines()
{
    ...
    foreach ($fs->serviceitems as $lino => $li) {
        ...
        if (isset($li['ndcnum'])) {
            ...
            echo " ";
            echo "<select class='form-control' name='bill[" . attr($lino) . "][ndcuom]' style='background-color:transparent'>";
...
if ($GLOBALS['sell_non_drug_products']) {
    ...
    echo ($i <= 1) ? " <tr>\n" : "";
    echo "  <td width='50%' align='center' nowrap>\n";
    echo "   <select class='form-control' name='Products' style='width:96%' onchange='codeselect(this)'>\n";
...
echo "   <select class='form-control' name='Search Results' style='width:98%' " .
...
if (true) {
    ...
    echo "   <select class='form-control' name='pricelevel'";
    if ($isBilled) {
        echo " disabled";
    }
