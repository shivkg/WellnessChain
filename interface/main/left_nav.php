...
'pfb' => array(xl('Dashboard')  , 0, '../interface/patient_tracker/patient_tracker.php?skip_timeout_reset=1'),
'rcb' => array(xl('History')  , 0, '../interface/main/messages/messages.php?go=Recalls'),
'dotr' => array(xl('Doctors')  , 0, '../interface/usergroup/usergroup_admin.php?'),
'patrep' => array(xl('patient_report')  , 0, '../interface/patient_tracker/patient_report.php?'),
// 'updocu' => array(xl('document-upload')  , 0, '../interface/patient_tracker/document-upload.php?'),
...
'his' => array(xl('History')   , 0, 'patient_file/history/history.php'),
...
$(document).ready(function(){
  if(1 == <?php echo $GLOBALS['menu_styling_vertical'] ?>){
    ...
    $("#navigation-slide > li  > a#pfb0").prepend('<i class="fa fa-fw fa-tachometer fa-2x"></i>&nbsp;');
    $("#navigation-slide > li  > a#rcb0").prepend('<i class="fa fa-fw fa-calendar-minus-o fa-2x"></i>&nbsp;');
    $("#navigation-slide > li  > a#dotr0").prepend('<i class="fa fa-fw fa fa-user-md fa-2x"></i>&nbsp;');
    $("#navigation-slide > li  > a#patrep0").prepend('<i class="fa fa-file-text-o fa-2x"></i>&nbsp;');
    // $("#navigation-slide > li  > a#updocu0").prepend('<i class="fa fa-file-text-o fa-2x"></i>&nbsp;');
...
if (!$GLOBALS['disable_pat_trkr'] && !$GLOBALS['disable_calendar'] && acl_check('patients', 'appt')) {
    genTreeLink('RTop', 'pfb', xl('Dashboard'));
}
...
if (!$GLOBALS['disable_rcb'] && !$GLOBALS['disable_calendar'] && acl_check('patients', 'appt')) {
    genTreeLink('RBot', 'rcb', xl('History'));
}

if (!$GLOBALS['disable_rcb'] && !$GLOBALS['disable_calendar'] && acl_check('patients', 'appt')) {
    genTreeLink('RBot', 'patrep', xl('Patient Reports'));
}
// if (!$GLOBALS['disable_rcb'] && !$GLOBALS['disable_calendar'] && acl_check('patients', 'appt')) {
//     genTreeLink('RBot', 'updocu', xl('Upload Document'));
// }



if (!$GLOBALS['disable_rcb'] && !$GLOBALS['disable_calendar'] && acl_check('patients', 'appt')) {
    genTreeLink('RBot', 'dotr', xl('Doctors'));
}
...
<?php if ($GLOBALS['inhouse_pharmacy'] && acl_check('admin', 'drugs')) { ?>
  <li><a class="collapsed" id="invimg" ><span><?php xl('Pharmacy', 'e') ?></span></a>
    <ul>
...
<?php if (acl_check('patients', 'lab')) { ?>
  <li><a class="collapsed" id="proimg" ><span><?php xl('Labs', 'e') ?></span></a>
    <ul>
