 ...
 */
   ...
   // $sql= " SELECT l.title, l.begdate, l.enddate,l.referredby, p.note  FROM `lists` AS l INNER JOIN `prescriptions` AS p ON l.pid = p.patient_id WHERE l.pid = ? AND l.type = 'medication' ORDER BY begdate";
...
if (sqlNumRows($res)>0) {
    ...
    while ($row = sqlFetchArray($res)) {
        ...
        echo "<td>".text($row['destination'])."</td>";
