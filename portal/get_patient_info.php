...
// some validation
if (! isset($_POST['passaddon']) || empty($_POST['passaddon'])) {
    session_destroy();
...
DEFINE("COL_POR_USER", "portal_email");
...
if ($auth === false) {
    $logit->portalLog('login attempt', '', ($_POST['passaddon'] . ':invalid username'), '', '0');
    session_destroy();
...
if (empty($auth[COL_POR_SALT])) {
    if (SHA1($plain_code) != $auth[COL_POR_PWD]) {
        $logit->portalLog('login attempt', '', ($_POST['passaddon'] . ':pass not salted'), '', '0');
        session_destroy();
    ...
    if ($tmp != $auth[COL_POR_PWD]) {
        $logit->portalLog('login attempt', '', ($_POST['passaddon'] . ':invalid password'), '', '0');
        session_destroy();
        header('Location: ' . $landingpage . '&w&p');
        exit();
    }
...
$_SESSION['email'] = $_POST['passaddon'];
...
if ($userData = sqlQuery($sql, array(
    ...
    if (empty($userData)) {
        $logit->portalLog('login attempt', '', ($_POST['passaddon'] . ':not active patient'), '', '0');
        session_destroy();
        header('Location: ' . $landingpage . '&w');
        exit();
    }
    ...
    if ($userData['email'] != $_POST['passaddon']) {
        $logit->portalLog('login attempt', '', ($_POST['passaddon'] . ':invalid email'), '', '0');
        session_destroy();
        header('Location: ' . $landingpage . '&w');
        exit();
    }
    ...
    if ($password_update) {
        ...
        if (! (empty($_POST['pass_new'])) && ! (empty($_POST['pass_new_confirm'])) && ($code_new == $code_new_confirm)) {
            ...
            $logit->portalLog(
                'password update',
                $auth['pid'],
                ($_SESSION['email'] . ': ' . $_SESSION['ptName'] . ':success')
            );
        }
    }
    ...
    if ($authorizedPortal) {
        ...
        $logit->portalLog('login', $_SESSION['pid'], ($_SESSION['email'] . ': ' . $_SESSION['ptName'] . ':success'));
    } else {
        $logit->portalLog('login', '', ($_POST['passaddon'] . ':not authorized'), '', '0');
        session_destroy();
        header('Location: ' . $landingpage . '&w');
        exit();
    }
